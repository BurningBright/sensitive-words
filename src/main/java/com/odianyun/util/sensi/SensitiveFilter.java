...
public class SensitiveFilter implements Serializable {
    ...
    /**
     * whether the string contain sensitive word or not
     *
     * @param sentence string to be check
     * @return is contain or not
     * @author burningbright
     * @date 2018年10月27日 下午21:00:00
     */
    public boolean contains(String sentence) {
        StringPointer sp = new StringPointer(sentence);
        for (int i = 0, step = 1; i < sp.length - 2; i += step) {
            step = 1;
            int hash = sp.nextTwoCharHash(i);

            /* may not the first node */
            SensitiveNode node = nodes[hash & (nodes.length - 1)];
            /* skip legal sentence quicker */
            if (node != null) {
                /* next pointer's position */
                int mix = sp.nextTwoCharMix(i);
                for (; node != null; node = node.next) {
                    if (node.headTwoCharMix == mix) {
                        NavigableSet<StringPointer> desSet = node.words.headSet(sp.substring(i), true);
    ...
    /**
     * remove word
     * not delete the empty nodes yet
     * @param word word to be remove
     * @return remove success or not
     * @author burningbright
     * @date 2018年10月27日 下午21:00:00
     */
    public boolean remove(String word) {
        if (word == null || word.trim().length() < 2) return false;
        if (word.length() == 2 && word.matches("\\w\\w")) return false;
    ...
    private boolean remove(SensitiveNode node, StringPointer sp) {
        ...
        /* if got the target remove word from list */
        int mix = sp.nextTwoCharMix(0);
        if (node.headTwoCharMix == mix) {
